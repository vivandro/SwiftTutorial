 ...
 ******************************************************************************/
class Celsius {
    let temperatureInCelsius: Double // Constant property
    init(fahrenheit: Double) {
        // You are allowed to modify conastant properties only during init.
        temperatureInCelsius = (fahrenheit - 32.0) / 1.8
        // temperatureInCelsius = 22.0 // ERROR: Immutable properties can only be assigned a value once.
    }
...
func printCollectionTypes2(aCollection: [ABase]) -> [String] {
    ...
    for item in aCollection {
        if let _ = item as? BDerived {
            result.append("+BDerived")
        } else if let _ = item as? CDerived {
            result.append("+CDerived")
...
func printThings(aCollection: [Any]) -> [String] {
    ...
    for item in aCollection {
        ...
        case _ as Int: // We can ignore the matched value
            result.append("~Int")
        case _ as Double:
