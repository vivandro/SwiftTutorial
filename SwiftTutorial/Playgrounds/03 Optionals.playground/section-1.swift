...
if let j = iOpt {
    println("Value extracted from iOpt is useable as \(j)")
}
...
if let str = numString[10] {
    ...
    println("10: No conversion to word")
    numString[10] = "Ten"
}
...
if (optNumString?[200] = "Two Hundred") != nil { // Success
    println("We did some great work over there, team!")
} else { // Failure
    println("In the end, we are just as strong as the weakest link!")
}
...
case nil:
    println("No value to call lowercaseString on")
case .Some(let someStr):
    println("found \(someStr)")
default:
    break
}
...
case nil:
    break
case .Some(let someStr):
    println("found [\(someStr)]. result should be [\(someStr.lowercaseString)]")
default:
    break
}
